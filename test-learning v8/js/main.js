...
/**
 * 更新頁面上的使用者姓名
 * 註: localStorage.userName 只在登入當下寫入一次，若行政端事後修改了使用者姓名，
 * 快取就會過期。因此這裡先用快取值立即顯示 (避免畫面閃爍空白)，
 * 再向後端拿最新資料校正畫面並更新快取，讓修改能在下次載入頁面時同步。
 */
async function updateUserName() {
  const cachedName = localStorage.getItem('userName');
  applyUserName(cachedName);

  if (typeof api === 'undefined' || !localStorage.getItem('authToken')) return;

  try {
    const result = await api.auth.getProfile();
    const freshName = result?.data?.name;
    if (freshName && freshName !== cachedName) {
      localStorage.setItem('userName', freshName);
      applyUserName(freshName);
    }
  } catch (error) {
    // 取得最新資料失敗時，保留快取姓名即可，不影響頁面其他功能
    console.error('更新使用者姓名失敗:', error);
  }
}
